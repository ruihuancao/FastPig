...
import com.ruihuan.fastupdate.utils.ColorUtil;
import com.ruihuan.fastupdate.utils.DrawableUtil;
import com.ruihuan.fastupdate.view.NumberProgressBar;
...
public class UpdateDialogFragment extends DialogFragment implements View.OnClickListener {
    ...
    private int mDefaultPicResId = R.drawable.fastupdate_top_bg;
...
//        setStyle(DialogFragment.STYLE_NO_TITLE | DialogFragment.STYLE_NO_FRAME, 0);
        setStyle(DialogFragment.STYLE_NO_TITLE, R.style.FastUpdateDialog);
...
//        getDialog().getWindow().addFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL);
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fastupdate_dialog, container);
    }
...
//            }
        ...
        if (i == R.id.btn_ok) {
            ...
            if (flag != PackageManager.PERMISSION_GRANTED) {
                if (ActivityCompat.shouldShowRequestPermissionRationale(getActivity(), Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                    ...
                    Toast.makeText(getActivity(), getResources().getString(R.string.fastupdate_premission_tip), Toast.LENGTH_LONG).show();
                } else {
                    // 申请授权。
    ...
    public void cancelDownloadService() {
        if (mDownloadBinder != null) {
            // 标识用户已经点击了更新，之后点击取消
            mDownloadBinder.stop(getResources().getString(R.string.fastupdate_download_cancel));
        }
    }
        ...
        if (mUpdateApp != null) {
            ...
            binder.start(mUpdateApp, new DownloadService.DownloadCallback() {
                ...
                public void onProgress(final float progress, long totalSize) {
                    mNumberProgressBar.post(new Runnable() {
                        @Override
                        public void run() {
                            if (!UpdateDialogFragment.this.isRemoving()) {
